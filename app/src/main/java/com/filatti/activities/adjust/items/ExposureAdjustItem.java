...
public class ExposureAdjustItem extends AdjustItem<ExposureAdjust> {
    private SliderAdapter mSliderAdapter;

    public ExposureAdjustItem(ExposureAdjust effect) {
        super(effect);
    }
    ...
    public View getView(Context context, ViewGroup rootView) {
        ...
        ViewGroup viewGroup =
                (ViewGroup) inflater.inflate(R.layout.exposure_item_view, rootView, false);
        ...
        mSliderAdapter = new SliderAdapter(textView, sliderView, -300, 300);
    ...
    private class SliderAdapter extends SliderActionAdapter {
        ...
        protected void setToEffect(int valueFromSlider) {
            float exposure = valueFromSlider / 100.0f;
            try {
                mEffect.setExposure(exposure);
            } catch (EffectException e) {
                Timber.e(e, "Failed to set exposure %f", exposure);
            }
        }
        ...
        protected int getFromEffect() {
            return convertFromEffect(mEffect.getExposure());
        }

        @Override
        protected int getInitFromEffect() {
            return convertFromEffect(mEffect.getInitExposure());
        }
